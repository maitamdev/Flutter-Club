...
export default function LoginPage() {
  ...
  const handleRedirect = async (uid: string) => {
    ...
    if (exists) {
      ...
      // Nếu là member thì vào dashboard, nếu không có role thì request access
      if (userData && userData.role === 'member') {
        router.push('/dashboard')
      } else if (userData && (userData.role === 'admin' || userData.role === 'trainer')) {
        // Admin/Trainer đăng nhập nhầm trang -> chuyển về admin login
        toast({
        ...
        <CardHeader className="text-center space-y-4">
          ...
          <div>
            ...
            <CardDescription className="mt-2">
              FT-Club Hub - Câu lạc bộ Flutter
            </CardDescription>
          </div>
        </CardHeader>
